	...
	const addWithPersent = () => {
		const foods = food.map((food) => {
			if (food.id !== 2 && food.id !== 4) {
				return {
					...food,
					price: Math.floor(((+food.price * 30) / 100) + +food.price)
				}
			} 
			return food
	...
	return (
		<div>
			...
			{food.map((food, index) => (
				...
				key={food.id ?? index}
				disabled={!food.id} 
				onClick={() => deleteId(food.id)}>Delete{food.name}</button>
			))}
			...
			<button disabled={!food[0].id} onClick={addWithPersent}>AddWithPersent</button>
